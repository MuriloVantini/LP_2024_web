<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('Atualização de Pagamento - Código: ' . $pagamento->id) }}
        </h2>
    </x-slot>
    @section('content')
        <br><br>
        <form class="max-w-md mx-auto" action="{{ route('pagamentos.update', $pagamento->id) }}" method="POST">
            @csrf
            @method('PUT')
            <label for="date-picker" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Data de
                pagamento</label>
            ...
            <div class="relative z-0 w-full mb-5 group">
                <input type="number" name="valor_pago" id="valor_pago"
                    value="{{ old('valor_pago', $pagamento->valor_pago) }}" step="0.01"
            ...
            <div class="relative z-0 w-full mb-5 group">
                <label for="metodoPagamento" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Método de
                    pagamento</label>
                <select name="metodo_pagamento" id="metodoPagamento" step="0.01"
                    class="py-2.5 px-1 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder=" " required>
                    <option class="dark:bg-gray-900" value="cartao"
                        {{ $pagamento->metodo_pagamento == 'cartao' ? 'selected' : '' }}>Cartão</option>
                    <option class="dark:bg-gray-900" value="dinheiro"
                        {{ $pagamento->metodo_pagamento == 'dinheiro' ? 'selected' : '' }}>Dinheiro</option>
                    <option class="dark:bg-gray-900" value="transferencia"
                        {{ $pagamento->metodo_pagamento == 'transferencia' ? 'selected' : '' }}>Transferência</option>
                </select>
            </div>
            <button type="submit"
