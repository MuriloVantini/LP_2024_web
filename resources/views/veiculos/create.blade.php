<x-app-layout>
    ...
    @section('content')
        <br><br>
        <form class="max-w-md mx-auto" action="{{ route('veiculos.store') }}" method="POST">
            @csrf
            ...
            <div class="relative z-0 w-full mb-5 group">
                <label for="preco_diaria" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Preço da diária</label>
                <input type="number" name="preco_diaria" id="preco_diaria" step="0.01"
                    class="py-2.5 px-1 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    placeholder=" " required />
            </div>
            <button type="submit"
